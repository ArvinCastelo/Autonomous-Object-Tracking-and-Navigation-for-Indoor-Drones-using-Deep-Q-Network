...
class pid_velocity_fixed_height_controller():
    ...
    def pid(self, dt, desired_vx, desired_vy, desired_yaw_rate, desired_altitude, actual_roll, actual_pitch, actual_yaw_rate,
        ...
        if dt <= 0 or not isinstance(dt, float) or dt != dt:
            print("[PID] Invalid dt:", dt)
            return [0, 0, 0, 0]
        for var_name, value in [
            ...
            ("roll", actual_roll),
            ("pitch", actual_pitch),
            ("yaw_rate", actual_yaw_rate)
        ]:
            if value != value:  # NaN check
                print(f"[PID ERROR] NaN input detected: {var_name} = {value}")
                return [0, 0, 0, 0]
        ...
        alt_command = gains["kp_z"] * alt_error + gains["kd_z"] * alt_deriv + \
            gains["ki_z"] * np.clip(self.altitude_integrator, -2, 2) + 10
