...
DATA_DIR           = "dataset10"   # where rollouts are stored
        ...
        with open(filename, "w", newline="") as f:
            ...
            while step_idx < MAX_EPISODE_STEPS and not done:
                ...
                if state["front_dist"] > WALL_THRESHOLD:
                    explore_ids.append(ctrl.forward)
                if state["back_dist"] > WALL_THRESHOLD:
                    explore_ids.append(ctrl.backward)
                if state["left_dist"] > WALL_THRESHOLD:
                    exploit_ids.append(ctrl.left)
                if state["right_dist"] > WALL_THRESHOLD:
                    exploit_ids.append(ctrl.right)


                # Always allow hover, ascend, descend
                explore_ids.extend([ctrl.hover,
                                      ctrl.ascend,
                                      ctrl.descend,
                                      ctrl.yaw_left,
                                      ctrl.yaw_right,
                ...
                exploit_ids.extend([ctrl.halfspinleft,
                                    ctrl.halfspinright])

                # Pick an action by epsilon greedy
                if random.random() > EPSILON:
                    action_id = random.choice(exploit_ids)
                else:
                    action_id = random.choice(explore_ids)
